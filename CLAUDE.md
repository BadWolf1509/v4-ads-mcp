...
**49 MCP tools** registered: 23 read + 25 mutations + `apply_change`. Production revision `v4-ads-mcp-00199-l8v` (Sprint 3b.26.1 F42 fix; /health 200, CI green).
...
- **Phase 3b restante** — Sprints 3b.24, 3b.25, 3b.26 shipados em produção (49 MCP tools). **Roadmap reordenado em 2026-05-19 pós-dogfood MO-JP** (ver [dogfood-2026-05-19-mestre-da-obra-jp-cleanup-massivo.md](docs/operacao/dogfood-2026-05-19-mestre-da-obra-jp-cleanup-massivo.md) §priorização ICE — combo prazo V4 23/05 + B1 HIGH catalogado como F43):
  - **Sprint 3b.27 candidate** — combo `update_customer_conversion_goal` (#2 ICE 540, prazo MO 23/05 Opção C SIMPLIFICADA) + **B1 pre-flight negative check em `update_keyword_status`** (#1 ICE 630, [F43](docs/operacao/findings-catalog.md) HIGH). Tool count 49 → 50 + 1 fix. Padrão técnico unificado: pre-flight async via GAQL (família Sprint 3b.8).
  - **Sprint 3b.28 candidate** — `upload_customer_match_list` + A4 investigation (audience exclusion mechanism aberto desde Sprint 3b.4/3b.5; LGPD-heavy: SHA-256 hashing client-side; ver [findings-catalog.md](docs/operacao/findings-catalog.md) §A4). Runbook esqueleto já existe em [phase-3b-28-bootstrap.md](docs/operacao/phase-3b-28-bootstrap.md) (gerado 2026-05-19 via subagent `smoke-runbook-generator`). Tool count 50 → 51.
  - **Sprint 3b.29 candidate** — `remove_keyword/campaign/ad_group/ad/asset` bundle (cleanup ops + cleanup de ~75 entities criadas em Nutry sandbox durante smokes 3b.24-3b.26).
  - **Sprint 3b.30+ candidates (família "audit_*" curadas, do ICE dogfood MO-JP)** — `audit_quality_score` (#4 ICE 504), `audit_competitor_keywords` (#6 ICE 432), `audit_zombie_keywords`, `audit_orphan_smart_actions`, `audit_negative_criterion_overlap`, `audit_assets_parity_between_campaigns`. Família coesa — vale dispatcher shared. Recorrência alta em sessões de cirurgia (próxima MO + outras contas V4 herdadas).
