...
**Tokens válidos:** v4-ads Bearer (procedure abaixo; **Bearers antigos seguem válidos** pós-migração — validados por hash no DB compartilhado). Meta system-user token **all-targets** no secret `meta-system-user-token` (app *V4 Ads MCP* `1522411803012799`, SU `v4-ads-mcp-integracao` `61590110716028`, não expira). Meta OAuth pessoal do Wellington **expirado desde 27/07/2026** — dormente e inofensivo: as tools Meta rodam com o system-user token (Modelo B). O painel exibe "expira em … (0 dias)" pra uma data passada; cosmético, não corrigido.
...
**Decision gates:** **checkpoint volume Meta** (janela 06-25→07-10; 500 calls/15d → re-submit Full Access) — volume real 07-02 = **só 43 calls, 2,3% erro** → longe de 500; M.5 alimentaria OU estender · **soak Fase 2A→2B** → NÃO tombstonar (ver "Próximo sprint" acima) · ~~2026-07-25 reconectar Meta OAuth Wellington~~ — venceu sem ação e **não precisa** de ação (Modelo B cobre).
