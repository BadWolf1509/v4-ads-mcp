...
**Decision gates** (números reverificados no audit em 11/08, janela 30 dias, 1227 eventos): **checkpoint volume Meta** — **343 chamadas Meta/30d** (~172/15d) contra a régua de 500/15d. Subiu muito desde os 43 de 07-02, mas ainda não bate; M.5 fecharia a conta · **soak Fase 2A→2B** → ainda **NÃO tombstonar**: 134 chamadas aos 8 reports antigos contra **9** de `get_performance_breakdown` (~15:1). Saiu do zero, mas a migração não aconteceu · ~~2026-07-25 reconectar Meta OAuth Wellington~~ — **feito em 11/08**, válido até 10/10/2026 (era opcional de todo jeito: Modelo B cobre).

**Pendente operacional (AÇÕES HUMANAS — runbook detalhado no handoff 2026-07-02):**
1. **`lucassoares` nunca onboardou — não é cutover, é 1º acesso.** Verificado em 11/08: conta **ativa** desde 05/05, **25/25 contas Google concedidas**, mas `last_seen_at` **vazio** (nunca entrou no painel) e **0 eventos em 90 dias**. Não há setup antigo pra migrar. O lado servidor está pronto; **falta só ele executar** — e isso ninguém faz por ele, porque exige o login Google dele e a criação da sessão (o Bearer só é válido se emitido pela UI, pelo próprio gestor). Passos: `/login` com a conta @v4company.com → `/accounts` → "Conectar nova conta" → `/sessions` → criar sessão e copiar o Bearer (aparece **uma vez**) → colar no cliente com o snippet de `/help` (que já mostra a URL nova). **Antes de tratar como tarefa, confirme com ele se vai usar** — 90 dias sem um único acesso sugere que talvez não. `anderson` está ativo (1º acesso feito, última chamada 11/08 18:49).
