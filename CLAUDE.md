...
**/health 200, CI green.** **16 web pages** em prod. **Q8 invite-only allowlist** ativo. **50 findings catalogados** (F1-F52 + A1-A6 + D1, alguns IDs skipped): [`findings-catalog.md`](docs/operacao/findings-catalog.md). **Smoke real M.2b 8/8 PASS** com F48 (FacebookSession factory `a281c00`) + F49 (button macro type `fe9a976`) caught/fixed em prod. **Smoke real 3b.38 5/5 PASS** F52 ad_group_status field + F23 clamp validados bit-a-bit em conta MO-JP+CAB (280 zumbis = 170 REMOVED + 110 ENABLED match exato dogfood). F50/F51 retrospective trace de F33/F37 (já fixados desde 2026-05-18). D1 decision-not-bug: Meta App Review Standard Tier rejeitado, Dev Mode permanente OK pra V4 LS&Co interno.

### Pending / future

- **Meta App Review RESPONDIDO 2026-05-25 10:58 GMT-3** — `public_profile` ✅ APROVADA, `Marketing API Access Tier` ❌ REJEITADA ("insufficient Ads API calls in the last 15 days"). **Decisão Caminho A Dev Mode permanente** — V4 LS&Co (Wellington + 3 colab futuros) = 4 users ≤ 25 cap Dev Mode. Standard Tier não-necessário pra uso interno. **Roadmap M.3-M.25 segue normal** — Dev Mode permite SDK + rate limits + todas tools idênticas; única diferença é "quem pode autenticar OAuth no app" (apenas admins/testers). **Decision gate atualizado:** continuar M.3 = ≥3 calls/semana dogfood Wellington; senão pivot Google. **Action quando colab entrarem:** add como App Roles → Administrators no Meta Dev Console. Ver D1 finding.
- **Sprint 3b.39 candidates** (post-3b.38 ship): audit_negative_criterion_overlap, audit_assets_parity_between_campaigns, remove_* bundle, audit_log gap fix em `run_gaql`/`get_my_audit_log`/`get_my_rate_limit_status`, W2 `verify_campaign_state` ICE 280.
...
- **3 colaboradores V4 LS&Co como Meta App Administrators** quando time começar a usar — adicionar via Meta Dev Console > App Roles > Administrators (Dev Mode permite 25 admins/testers sem Standard Access Tier, conforme decisão Caminho A pós-App Review 25/05).
