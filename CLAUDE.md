...
**Última atualização:** 2026-07-22. Produção verde em **`v4-ads-mcp-00027-kmw`** (projeto GCP `v4-ads-mcp`, Wellington owner), `/health?deep=1` db=ok. **~64 MCP tools** (58 Google + 6 Meta), bucket ~23 always + 41 defer. **Sessão 07-22:** investigação de erros de produção (3 categorias, **0 bug de código**) + **F76** (500 intermitente `mcp_auth_error` por conexão DB stale → `run_with_reconnect`) + observabilidade (`level`→`severity` no Cloud Logging) + **deploy fail-well no smoke** (smoke F58 **dormente**) + regressão de acento (pin A7). Infra (07-04): **alerting**, **backup semanal→GCS**, **lockfile pinado**, **CI DB ~45-90s**; painel web com pacote de UI/UX (F74/F75).
...
6. **[07-23/24 — TIME-SENSITIVE] Verificar taxa de `mcp_auth_error`** pós-F76 (após janela ociosa da madrugada). Retry no ar desde `00026` (22/07 15:41 UTC); em ~2h só teve tráfego quente (0 erro), gatilho idle não ocorreu. Sinais: `db_dropped_connection_retry`>0 = prova positiva (retry recuperou) · `mcp_auth_error` em `00026`+ = raro caso das 2 tentativas falharem (subir `attempts`/baixar lifetime). Baseline 5/14d. Query no handoff 07-22.
7. **Smoke F58 dormente** (gestor smoke excluído 07-14) — deploys OK via fail-well; re-armar recriando gestor smoke sem grants + repor `SMOKE_MCP_BEARER` no GitHub se quiser a proteção ativa (memory `ci-smoke-bearer-dormant-2026-07`).

**Quando ler outros docs:** [bug suspeito → `findings-catalog.md`] [o que shipou / detalhe sprint → `sprint-history.md`] [última sessão → `session-2026-07-22-handoff.md`] [migração GCP → `session-2026-06-30-handoff.md`] [executar pendente → spec+plan em `docs/superpowers/`].

## Read these first when continuing work

```
docs/operacao/findings-catalog.md            # ★ Bug history (F1-F76 + A1-A7 + D1-D3) — scan antes de design mutate/query
docs/operacao/sprint-history.md              # ★ Tabela por sprint (3b.1→3b.40 + M.4 + Fase 2A + Meta); F64+ só nos handoffs/catalog
docs/operacao/session-2026-07-22-handoff.md  # ★ Erros de produção (3 cat, 0 bug) + F76 (500 por conexão DB stale → run_with_reconnect) + observabilidade level→severity + smoke fail-well dormente + regressão acento; pendente: D+1/D+2 mcp_auth_error + Meta #200 CA-ROL GEAN
...
- Don't fazer read no hot-path (ex.: resolução de sessão) com `pool.acquire()` cru — use `connection.run_with_reconnect(op)` (asyncpg NÃO faz pre-ping; conexão idle fechada pelo Supabase só falha no próximo statement → 500 intermitente, F76). Retry só em read idempotente; mutação NÃO leva retry cego (pode ter commitado). Log Cloud Logging usa `severity` (não `level`) — `add_cloud_logging_severity` já cobre no pipeline JSON.
