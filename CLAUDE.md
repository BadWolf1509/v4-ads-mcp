...
| Sprint 3b.1 — `add_negatives_from_search_terms` + `get_change_history` | ✅ 2026-05-11 | 11 commits ([f156d99..3549778](https://github.com/BadWolf1509/v4-ads-mcp/compare/c660c6c..3549778)); smoke runbook pending [`phase-3b-1-bootstrap.md`](docs/operacao/phase-3b-1-bootstrap.md) |

**34 MCP tools** registered: 21 read + 12 mutations + `apply_change`.
...
- **Sprint 3b.1 smoke runbook** — [`docs/operacao/phase-3b-1-bootstrap.md`](docs/operacao/phase-3b-1-bootstrap.md) aguardando execução manual por `wellinton.ribeiro@v4company.com` em conta V4 real. 5 testes (happy path, idempotência, filter precision, 30-day enforcement). Sign-off oficial = todos os checkboxes preenchidos. Production /health 200 OK + smoke test do workflow já passou — falta validar comportamento real com Google Ads API (especialmente `CRITERION_EXISTS` partial_failure text e `change_event.client_type` enum names).
- **Phase 3b restante** — 13 mutation tools faltantes (`create_campaign`, `create_ad_group`, `add_keywords`, `update_keyword_match_type`, `create_rsa`, `update_rsa`, `update_ad_status`, `create_asset`, `link_assets`, `apply_audience`, `upload_customer_match_list`, `create_conversion_action`, `import_offline_conversions`, `bulk_pause_by_query`) + 2 utilities (`get_my_rate_limit_status`, `get_my_audit_log`). Sprint 3b.2 candidato (médio risco, baixo quota): `bulk_pause_by_query` + `update_ad_status`. Sprints 3b.3-3b.4 (alto quota — bloqueado por Standard Access): creates de campanha/RSA. Ordem subsequente priorizada por sinal da operação piloto.
