...
| Sprint 3b.16 — `create_rsa` (segundo create-pattern do MCP) | ✅ 2026-05-12 | 5 commits ([044ea69..592bcc2](https://github.com/BadWolf1509/v4-ads-mcp/compare/24e4f8e..592bcc2)); **1 new MCP tool (count 42 → 43):** batch creation de 1-5 Responsive Search Ads (RSAs) por chamada em existing ad_groups. Always-CONFIRM. Schema: `ad_group_id` (parent) + `headlines` (3-15 × 30 chars) + `descriptions` (2-4 × 90 chars) + `final_urls` (1+) + `path1`/`path2`? (max 15 chars cada) + `status`? (PAUSED default). **JSONSchema enforces Google's structural rules declaratively** (rejected pré-runtime). Pre-flight helper `validate_parent_ad_groups_for_rsa_create` em `_common.py`: ad_group existence + status != REMOVED + parent campaign channel ∈ {SEARCH, SEARCH_PARTNERS}. Builder `build_create_rsa` em `mutates/ads.py` usa `responsive_search_ad.headlines.add()` pattern (proto-plus repeated message field). **F13 resource_names benefit auto-inherited** (Sprint 3b.15) — first integration test em `test_create_rsa.py` asserts resource_names matches mock. **Bonus fixture extension:** ProtoFieldCapture (`tests/unit/fixtures/proto_capture.py`) ganhou `_RepeatedCapture` + `field_count(path)` helper para repeated proto field assertions (reusable em future creates: campaign, assets). 15 tests totais (5 helper + 6 ProtoFieldCapture builder + 3 tool + 1 integration). ~255 LOC source + ~500 LOC tests + ~90 LOC smoke runbook. Smoke runbook `phase-3b-16-bootstrap.md` (5 test cases incluindo F13 production validation + schema rejection paths) pending Wellington execute em Nutry sandbox. **11ª sprint consecutiva sem novos bugs no smoke** (assuming smoke passes). |

**43 MCP tools** registered: 23 read + 19 mutations + `apply_change`.
