...
**50 MCP tools** registered: 23 read + 26 mutations + `apply_change`. Production revision `v4-ads-mcp-00204-86d` (Sprint 3b.27.1 F44 fix; /health 200, CI green).
...
- **Phase 3b restante** — Sprints 3b.24, 3b.25, 3b.26, 3b.27 shipados em produção (50 MCP tools). Decisão de escopo: Sprint 3b.27 entregou `update_conversion_action` (não `update_customer_conversion_goal` como originalmente planejado) — pesquisa context7 mostrou que `ConversionAction.primary_for_goal=false` é o caminho cirúrgico pra rebaixar action específica (vs categoria-wide). Decision documented in [`docs/superpowers/specs/2026-05-19-sprint-3b-27-update-conversion-action-plus-b1-fix-design.md`](docs/superpowers/specs/2026-05-19-sprint-3b-27-update-conversion-action-plus-b1-fix-design.md).
  - **Sprint 3b.28 next-in-queue** — `upload_customer_match_list` + A4 investigation (audience exclusion mechanism aberto desde Sprint 3b.4/3b.5; LGPD-heavy: SHA-256 hashing client-side; ver [findings-catalog.md](docs/operacao/findings-catalog.md) §A4). Runbook esqueleto já existe em [phase-3b-28-bootstrap.md](docs/operacao/phase-3b-28-bootstrap.md) (gerado 2026-05-19 via subagent `smoke-runbook-generator`). Tool count 50 → 51.
