...
| Sprint 3b.1 → 3b.36 (36 sprints) | ✅ 2026-05-04→21 | All shipped + signed-off em conta real. **Detail per sprint:** [`sprint-history.md`](docs/operacao/sprint-history.md). **Bug history (42 findings, F1-F46):** [`findings-catalog.md`](docs/operacao/findings-catalog.md). |

**56 MCP tools** registered: 28 read + 27 mutations + `apply_change`. Production post-Sprint 3b.36 — `audit_zombie_keywords` 56ª tool (ICE 315 cleanup massivo dogfood 19/05). Smoke 6/6 PASS — caso real MO-JP **279 zombies** detectados (93 só no ad_group DELL), ML Antiguidades **248 zombies** (e-commerce long-tail). **Total 527 zombie keywords cleanup-ready em 2 contas.** Zero F-findings novos. Sprint 3b.35 — `audit_goal_attribution` shipped. /health 200, CI green.
...
- **Sprint 3b.37 candidate (next-in-queue):** audit_orphan_smart_actions (#12 ICE 288), audit_negative_criterion_overlap, audit_assets_parity_between_campaigns OR remove_* bundle OR audit_log gap fix OR W2 `verify_campaign_state` (ICE 280). audit_zombie_keywords shipped Sprint 3b.36. WATCH non-blocking 3b.36: default limit=200 estoura MCP cap em contas com 500+ keywords zombi (workaround: passar limit=50). V1 candidate bump default limit=100.
