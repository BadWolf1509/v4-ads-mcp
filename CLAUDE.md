...
| Sprint 3b.14 — `create_ad_group` (primeiro create-pattern do MCP) | ✅ 2026-05-12 | 5 commits ([ef9868a..6d914ac](https://github.com/BadWolf1509/v4-ads-mcp/compare/ebe3262..6d914ac)); **1 new MCP tool (count 41 → 42, primeiro create shipado):** batch creation de 1-10 ad_groups por chamada, always-CONFIRM (creates sensitive per spec §7.1, match `remove_audience` precedent). Schema: campaign_id (parent) + name (1-255) + type? (`SEARCH_STANDARD` default | `SHOPPING_PRODUCT_ADS`) + status? (`PAUSED` default | `ENABLED`) + cpc_bid_micros? optional. **Full pre-flight via new helper** `validate_parent_campaigns_for_ad_group_create` em `_common.py`: campaign existence + status != REMOVED + channel type compat (SEARCH_STANDARD→SEARCH, SHOPPING_PRODUCT_ADS→SHOPPING) + F12 lesson (cpc_bid_micros só em MANUAL_CPC/ENHANCED_CPC). New builder `build_create_ad_group` em `mutates/ad_groups.py` usa `type_` (proto reserved-word handling) + defaults. 14 testes totais (5 helper + 5 ProtoFieldCapture builder + 3 tool + 1 integration full cycle). ~240 LOC source + ~470 LOC tests + ~80 LOC smoke runbook docs. Smoke runbook em `phase-3b-14-bootstrap.md` (6 test cases incluindo pre-flight rejections + happy paths + non-idempotency confirmation) pending Wellington execute em sandbox. **9ª sprint consecutiva sem novos bugs no smoke** (assuming smoke passes). |

**42 MCP tools** registered: 23 read + 18 mutations + `apply_change`.
