...
| Sprint 3b.25 — `create_and_link_assets` (6º create-pattern, asset CRUD bundle) | ✅ shipped + signed-off em Nutry sandbox (15/15 PASS após 2 fix iterations) | Production revisions `v4-ads-mcp-00188-cwv` → `00191-f56` (Sprint 3b.25.1) → final (Sprint 3b.25.2). Smoke runbook signed-off ([`phase-3b-25-bootstrap.md`](docs/operacao/phase-3b-25-bootstrap.md)) — spec in [docs/superpowers/specs](docs/superpowers/specs) + plan in [docs/superpowers/plans](docs/superpowers/plans). **1 new MCP tool (count 47 → 48):** `create_and_link_assets` union, asset creation + immediate attachment in chained ops. Asset types: SITELINK, CALLOUT, STRUCTURED_SNIPPET, CALL, PROMOTION (5 types × 3 attachment levels = 15 combinations; attachment_levels: CUSTOMER, CAMPAIGN, AD_GROUP). Always-CONFIRM. Schema: assets array (1-20 per batch) + target_type + target_id (campaign_id or ad_group_id, CUSTOMER level auto-discovers via customer_id) + optional performance_max (for PROMOTION). **V4 invariants:** BR country code hardcoded, PT language auto-added, currency=BRL in Promotion, micros formula enforcement (e.g., 20% → 200_000 micros). **Architecture:** chained mutation pattern (Sprint 3b.19B established), N+M+K ops em single MutateGoogleAdsRequest — N asset creates + M asset_group creates (per level) + K attachment ops. Proto field validation via context7 context::query-docs per asset type (empirical per-value probing Sprint 3b.19A.1 lesson). ~17 unit tests (8 tool + 9 builder) + ~18 new builder tests + 2 integration. Pre-flight: none (trust Google runtime errors per F14 lesson). **Smoke runbook:** 15 tests Nutry sandbox (1163862076 campaigns inherited from 3b.24) — T1-T2 SITELINK CUSTOMER/CAMPAIGN, T3 SITELINK AD_GROUP (rare probe), T4-T5 CALLOUT, T6-T7 STRUCTURED_SNIPPET (header validation), T8-T9 CALL, T10-T11 PROMOTION (micros formula assertion), T12 batch mixed types, T13-T14 schema regression, T15 response cap (F22-equivalent boundary). T3 + T9 may surface F-findings (predicts rare attachment combos). Clean smoke = 12+/15 PASS. **Closes roadmap foundation** — asset CRUD + 3b.26 (offline conversions) + 3b.27 (customer match) + 3b.28 (remove ops) = onboarding complete. |
| Sprint 3b.26 — `import_offline_conversions` (49th tool, first ConversionUploadService dispatcher) | ✅ shipped + signed-off em Nutry sandbox (9/12 PASS após 1 fix iteration + 3 deferred F41) | Production revisions `v4-ads-mcp-00196-h22` → `v4-ads-mcp-00199-l8v` (Sprint 3b.26.1 F42 fix). Smoke runbook signed-off ([`phase-3b-26-bootstrap.md`](docs/operacao/phase-3b-26-bootstrap.md)). **1 new MCP tool (count 48 → 49):** `import_offline_conversions` dispatcher, first tool to use ConversionUploadService instead of GoogleAdsService.mutate. Converts offline conversion events (e.g., WhatsApp leads, CRM callbacks) to Google Ads tracking via GCLID-based attribution. Always dry-run CONFIRM (spec §7.1). Schema: conversion_upload (required gclid + conversion_action_id + conversion_value_brl optional + conversion_datetime required ISO-8601 + conversion_custom_variable_id/value optional) + conversion_environment (WEB | APP, default WEB) + partial_failure (bool, default true). **V4 invariants:** currency_code=BRL hardcoded. **Architecture:** ConversionUploadService dispatcher pattern (first tool outside GoogleAdsService.mutate), creates conversion_upload proto objects, dispatches via dedicated service endpoint (not chained in MutateGoogleAdsRequest). Builder validates: gclid format (hex string validation via regex), conversion_datetime parseable as ISO-8601, conversion_value_brl ≥ 0 if provided. Partial failure mode allows partial batch success (F39 boundary class: when >50% batch fails, consider error signal). ~12 unit tests (8 tool + 4 builder) + 2 integration. Pre-flight: minimal (trust Google runtime validation on gclid format — no local validation for Google's proprietary gclid encoding). **Smoke runbook:** 12 tests Nutry sandbox (create UPLOAD_CLICKS ConversionAction pre-test, capture real gclids from test clicks, mix of valid/invalid scenarios) — T1-T3 pre-flight validation (gclid format, conversion_action_id existence, datetime parsing), T4-T6 happy paths (single conversion, batch with multiple gclids, custom variable injection), T7 partial failure (50% invalid gclids, verify success count + failure details), T8-T9 temporal constraints (future datetime rejection, stale gclid > 30d), T10-T11 batch dedup (duplicate gclid within single batch, server idempotency via gclid dedup), T12 schema regression (malformed conversions array). Clean smoke = 10+/12 PASS expected (T8 may reject future dates, T9 may accept stale gclids per Google's retention policy). **Closes 3b.26 roadmap** — offshore conversion import unblocked for V4 lead-gen workflow (WhatsApp/CRM → gclid capture → MCP upload → Google Ads reporting). Next: 3b.27 (customer match upload) + 3b.28 (remove ops). |

**49 MCP tools** registered: 23 read + 25 mutations + `apply_change`.
**Updated 2026-05-19 post-deploy:** Production revision `v4-ads-mcp-00196-h22` (Deploy verde, /health 200, CI green).
