...
**Última atualização:** 2026-07-23. Produção verde em **`v4-ads-mcp-00028-lvc`** (100% do tráfego; commit `23fccfe`; CI/deploy run `30047283991`), `/health?deep=1` db=ok e sem ERROR pós-deploy. **~64 MCP tools** (58 Google + 6 Meta), bucket ~23 always + 41 defer. **F76 validado em produção:** conexão stale detectada, retry-com-reacquire e request MCP 200. **F77 corrigido:** deep health deixou de usar `pool.acquire()` cru e agora usa `run_with_reconnect` com deadline global de 5s. Smoke autenticado F58 segue dormente.
...
**Quando ler outros docs:** [bug suspeito → `findings-catalog.md`] [o que shipou / detalhe sprint → `sprint-history.md`] [última sessão → `session-2026-07-23-handoff.md`] [migração GCP → `session-2026-06-30-handoff.md`] [executar pendente → spec+plan em `docs/superpowers/`].

## Context bootstrap (minimum)

1. Leia **somente** [`session-2026-07-23-handoff.md`](docs/operacao/session-2026-07-23-handoff.md) para estado de produção, encerramento F76/F77 e próximos passos.
...
- **ZERO JavaScript inline.** `script-src` roda sem `'unsafe-inline'` — atributo `on*=`/`hx-on` ou bloco `<script>` numa template é **bloqueado pelo browser**, silenciosamente. Todo comportamento vive em [`v4-panel.js`](src/web/static/v4-panel.js) como listener delegado, acionado por `data-v4-action` (`drawer-toggle`, `dropdown-toggle`, `row-toggle`, `dialog-open`/`dialog-close`, `copy`, `confirm`) ou por `data-v4-autosubmit` / `data-v4-submit-once` / `data-v4-filter` / `data-v4-matrix-filter`. Ação nova = entrada no mapa `ACOES` + guard que confere. CSS específico de página entra por `{% block head_extra %}`; **`style-src` ainda precisa de `'unsafe-inline'`** (o htmx injeta `.htmx-indicator`).
...
- **Don't escrever JS inline em template** (`onclick=`, `hx-on`, `<script>`): a CSP bloqueia e o handler morre calado. Use `data-v4-*` + listener em `v4-panel.js`. Vale também pra HTML montado dentro de string Jinja passada a macro (aspas escapadas escondem o handler de grep).
