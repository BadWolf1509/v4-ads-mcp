...
1. **Cutover: só falta `lucassoares`** — **zero eventos em 30 dias** no audit (verificado 11/08). `anderson` está ativo (1º acesso feito; 1 chamada, última 11/08 18:49). Runbook: trocar URL v4-ads no `~/.claude.json` → `…299432068772.southamerica-east1.run.app/mcp` (Bearer vale) + relogar no painel + restart. **Vale confirmar se o lucas ainda vai usar a ferramenta** — 30 dias sem nenhum uso pode ser cutover pendente ou desinteresse.
