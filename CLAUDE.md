...
| Sprint 3b.6 — `remove_audience` (closes audience CRUD) | ✅ 2026-05-12 | 5 commits ([eb3ed19..193442a](https://github.com/BadWolf1509/v4-ads-mcp/compare/825a830..193442a)); smoke runbook signed-off em conta real ([`phase-3b-6-bootstrap.md`](docs/operacao/phase-3b-6-bootstrap.md)) — production revision `v4-ads-mcp-00115-znm`. **Closes audience CRUD:** create (3b.4) + validation (3b.5) + delete (3b.6). Tool `remove_audience` com schema target_type + target_id singular + criterion_ids array (cap 100), sempre CONFIRM (spec §7.1 remove). Builder em `audiences.py` constrói paths inline (ad_group `~`-compound vs campaign flat). 6 ProtoFieldCapture builder tests + 9 tool tests + 1 integration. **Cross-cutting fix descoberto em T4** (`apply_change.py`): propagação de `__params_summary__` via payload key (mesmo pattern que `__partial_failure__` + `__target_count__`) — beneficia TODAS as CONFIRM-path tools com custom audit summaries. 6 de 6 smoke tests PASS + **real biz value:** Nutry sandbox ad_group `183008426336` zerado (4 orphan criteria removidas: 3 user_interest observation + 1 user_list exclusion mixed batch). Achado T5: Google silently succeeds remove de criterion já removed (applied_count=1, não NOT_FOUND error) — same class do Sprint 3b.3 A1 silent dedupe; defensive `_classify_partial` mapping continua útil como guard. |

**39 MCP tools** registered: 21 read + 17 mutations + `apply_change`.
...
- **Phase 3b restante** — 9 mutation tools faltantes (`create_campaign`, `create_ad_group`, `create_rsa`, `update_rsa`, `create_asset`, `link_assets`, `upload_customer_match_list`, `create_conversion_action`, `import_offline_conversions`) + 2 utilities (`get_my_rate_limit_status`, `get_my_audit_log`). `update_keyword_match_type` descartado por API immutability (Sprint 3b.3 finding). `remove_audience` shipado em Sprint 3b.6 — `remove_keyword`/`remove_campaign`/`remove_ad`/`remove_ad_group` ainda pendentes se demanda real surgir (gestor pode usar Google Ads UI por enquanto). **Sprint 3b.7 candidatos** (sem dependência de Standard Access): UX fixes bundle (UX-1 ROAS placeholder warning + UX-2 enum int → string + UX-3 search_terms status decode), pre-push integration test sweep (lesson 3b.5), ou dogfood expansion em conta nova. **Sprints 3b.8+** (alto quota — bloqueado por Standard Access): creates de campanha/RSA. Decisão de próximo sprint depende de sinal do dogfood + veredicto do Standard Access (case `26521440673`, esperado ~14/maio).
