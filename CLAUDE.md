...
- **Phase 3b restante** — 13 mutation tools faltantes (`create_campaign`, `create_ad_group`, `add_keywords`, `update_keyword_match_type`, `create_rsa`, `update_rsa`, `update_ad_status`, `create_asset`, `link_assets`, `apply_audience`, `upload_customer_match_list`, `create_conversion_action`, `import_offline_conversions`, `bulk_pause_by_query`) + 2 utilities (`get_my_rate_limit_status`, `get_my_audit_log`). **Sprint 3b.2 (próximo)**: `update_ad_status` + `bulk_pause_by_query`. `bulk_pause_by_query` MVP: hard cap **100 entidades por chamada** (rejeita acima — força refinar GAQL, não trunca silenciosamente), sempre dry-run + `apply_change`, telemetria `query_matched_count`/`applied_count`/`limit_hit` no audit_log para re-tune após 4 semanas de uso real. Sprints 3b.3-3b.4 (alto quota — bloqueado por Standard Access): creates de campanha/RSA.
