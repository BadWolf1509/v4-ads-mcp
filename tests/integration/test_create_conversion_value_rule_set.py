...
async def test_create_conversion_value_rule_set_full_cycle_returns_3_resource_names(
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            "FROM audit_log WHERE operation = 'create_conversion_value_rule_set' AND dry_run IS NOT TRUE"
        )
    ...
    async with pool.acquire() as conn:
        previews = await conn.fetch(
            ...
            "WHERE operation = 'create_conversion_value_rule_set' AND dry_run IS TRUE"
        )
