...
Modelo B: tool usa system-user token via run_meta_graph_get.
Nenhuma meta_oauth_connections row necessária — só manager + meta_ad_accounts + matrix grant.
"""
...
async def _seed_manager_with_access(
    db,
    *,
    account_status: int = 1,  # ATIVO
):
    """Seed manager + meta_ad_accounts + matrix grant.

    Modelo B: sem meta_oauth_connections — acesso via system-user token.
    """
    mid = uuid4()
...
async def test_meta_get_account_overview_happy_path(db):
    ...
    Modelo B: nenhuma meta_oauth_connections row necessária — só matrix grant.
    """
    from src.mcp.tools.meta_get_account_overview import meta_get_account_overview

    mid = await _seed_manager_with_access(db)
...
async def test_meta_get_account_overview_account_status_warning(db):
    ...
    mid = await _seed_manager_with_access(db, account_status=3)
