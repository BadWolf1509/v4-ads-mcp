...
>
> **Last updated:** 2026-05-21 (Sprint 3b.33 signoff — F46 NEW (GAQL BETWEEN end_date midnight bug) descoberto em smoke T3. Zero F-findings em 3b.29/3b.30/3b.31/3b.32. 42 unique findings (F1-F46 highest, alguns IDs skipped).)
...
| **F46** | MED | 3b.33 | OPEN | `change_event.change_date_time BETWEEN 'YYYY-MM-DD' AND 'YYYY-MM-DD'` em GAQL trata end_date como **midnight 00:00:00 start-of-day exclusive**, não inclusive end-of-day. Resultado: single-day window `start_date=end_date=2026-05-20` retorna **0 rows silenciosamente** (mesmo que dia tenha changes). Multi-day window também afetado — exclui changes de end_date depois do meio-dia. Discovered em smoke 3b.33 T3 (Pedro Vytor cluster 20/05 10:12-10:13 não retornou com window 19→20, retornou só com 19→21). Affects: `get_change_history` (3b.1), `get_negative_keywords_audit` via `negative_criterion_creations_query` (3b.21), `detect_drift` (3b.33). Fix candidates: (a) `change_history_query` builder converte `end_date` pra `end_date + 1 day` antes do BETWEEN (mudaria comportamento explicit start=end pra retornar dia inteiro); (b) usar `BETWEEN '{start}' AND '{end+1}'` + `change_event.change_date_time < '{end+1}'` constraint adicional pra semantics correto; (c) timestamp range `BETWEEN '{start} 00:00:00' AND '{end} 23:59:59'`. Workaround imediato sem fix: gestor passa end_date = day_after_target. Família design-gap-via-Google-API-semantics (semantic mismatch entre date-string e timestamp field). 15ª variante da silent-acceptance family (returns empty silently instead of erroring). [phase-3b-33-bootstrap.md] |
