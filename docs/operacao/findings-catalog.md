...
>
> **Last updated:** 2026-05-25 (Retrospective audit_log diagnosis — +2 findings novos: **F50** (audit_log id=140 confirma TypeError da F33 em produção — `get_type("X")()` no MAXIMIZE_CONVERSIONS sem target_cpa), **F51** (audit_log id=149 confirma AttributeError da F37 em produção — `campaign.start_date` campo inexistente em v24). 48 unique findings (F1-F51 + A1-A6, alguns IDs skipped).)
...
| Status | Count |
|---|---|
| **Fixed** (source code change) | 31 (added F50, F51 — retrospective audit_log confirmation of F33+F37) |
...
**Total findings tracked:** 48 (was 46 + F50/F51 — retrospective audit_log evidence).
