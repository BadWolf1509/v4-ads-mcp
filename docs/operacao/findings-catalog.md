...
>
> **Como ler:** ~5300 linhas, 545 KB, IDs de **F1 a F192** (com lacunas), mais A1-A7 e D1-D3. **Sem contagem de IDs, de propósito:** só 44 findings têm cabeçalho `## F<n>` próprio e os demais vivem dentro de outras entradas, então toda contagem já tentada aqui deu número diferente conforme o critério — faixa e tamanho são reproduzíveis, contagem não. Faça busca dirigida por palavra-chave (`GAQL`, `pool`, `Meta`, `audit`, `ContextVar`), nunca leitura integral. Entradas corrigidas trazem um bloco **✅ CORRIGIDO** com o que foi feito **e o que ficou deliberadamente de fora**.
...
## F155 (HIGH, CORRIGIDO em 2026-09-06) — guards sem primitivo comum: 18 varreduras artesanais, cada uma com o proprio defeito de cobertura
...
5. **Caminho relativo que devolve zero arquivos fora da raiz.** O guard do relogio (F141)
   ...
   **74** arquivos; rodando de `src/` ou de `tests/unit/`, **zero** — e zero arquivos varridos
   nao e "sem violacao", e o guard inteiro nao tendo rodado. **Segundo ocupante da mesma
   forma, achado so na revisao final desta Task 9 (antes do merge), nao pela varredura
   original:** `test_create_pending_audita_dry_run.py::test_toda_tool_que_cria_pendencia_declara_o_target_count_planejado`
   (F148, guard do 25o call-site de `create_pending`) tinha o identico
   `pathlib.Path("src/mcp/tools")` relativo seguido de `.glob("*.py")` direto, com a mesma
   medicao (74 arquivos da raiz, zero de `src/` ou de `tests/unit/`). Ver a nota logo apos a
   tabela abaixo.
...
**Os 17 da secao 3.1.1 da spec (autoridade vinculante), mais um 18o que a spec nao lista
(linha 18 abaixo — achado so nesta revisao, ver a nota logo apos a tabela).** A coluna
**defeito** abaixo e a logica de CASAMENTO que sobra em cada guard depois da conversao — a
dimensao de ESCOPO (recursivo, absoluto, com `EscopoVazioError`) ja esta corrigida nos 18,
uniformemente, desde este PR: e exatamente o que adotar o harness compartilhado entrega de
graca, e nao precisou esperar a frente de cada guard. O que resta, guard a guard, e a logica
de casamento especifica de cada um, e e essa logica que viaja marcada em **aperta em**. Os
tres `PR 0` abaixo foram fechados nesta propria rodada: F58 e F91 na Task 8, depois de provar
ausencia de violacao viva; a linha 18, na Task 9, por ser vacuidade de escopo (nao casamento) —
nao precisou da mesma prova, porque nao ha "aperto" de logica ali, so a correcao do caminho:
...
| 18 | `test_create_pending_audita_dry_run.py::test_toda_tool_que_cria_pendencia_declara_o_target_count_planejado` (F148) | caminho relativo com vacuidade silenciosa — mesma forma 5 da linha 5; ausente da tabela da spec, achado so nesta revisao | **PR 0 — fechado aqui** |

**A 18a linha, e o que ela confirma.** As primeiras 17 linhas sao as que a spec listou em
05/09, com 8 revisores paralelos. A 18a apareceu do lado de dentro: a revisao desta propria
Task 9, ainda antes do merge, achou que
`test_create_pending_audita_dry_run.py:134` (guard do F148, o 25o call-site de
`create_pending`) tinha o mesmissimo `pathlib.Path("src/mcp/tools")` relativo seguido de
`.glob("*.py")` direto do guard do relogio (item 5 da lista de formas, acima) — e a mesma
medicao: 74 arquivos da raiz do repo, zero de `src/` ou de `tests/unit/`. A forma 5 tem agora
**dois ocupantes** conhecidos, nao um. Convertido nesta mesma rodada para
`h.fontes_py(h.SRC / "mcp" / "tools")`, com uma prova de mordida propria: um teste novo
(`test_guard_do_target_count_deriva_escopo_do_harness_nao_de_path_relativo`) que le a ARVORE
da funcao-alvo — nao o comportamento dela, que fica identico rodando da raiz — e fica vermelho
se ela parar de chamar `h.fontes_py`. Verificado contra o codigo pre-fix
(`git show add570b:tests/unit/test_create_pending_audita_dry_run.py`, o commit anterior a esta
correcao): a mesma assercao, rodada contra aquela arvore, da `False`. Isto e a tese do
paragrafo de abertura desta entrada — "corrigir os 17 um a um... garantiria o 18" — confirmada
empiricamente e por dentro do proprio PR que fecha a classe: o 18o apareceu exatamente como
previsto, porque quem escreveu aquele guard (sessao do F148, anterior a este PR) nao sabia dos
...
**As cinco linhas que a tabela marca com um PR mas o paragrafo acima nao explica — F92 (linha
3), F83 (linha 4), DSN (linha 5), F113 (linha 8) e o guard sem nome de finding da linha 9 —
tambem tem motivo, so que mais tecnico que "e a mesma familia de PR":**

- **F92 (linha 3) vai com o painel (PR 5) porque a funcao que motivou o achado ja mudou de
  forma.** `pending_invites_count()` (`src/web/deps.py:84`) nao chama mais `get_pool`+`acquire`
  diretamente — foi migrada pra `connection.run_with_reconnect` (comentario `# F91 — 11 rotas
  admin chamam isto` no proprio arquivo), que e o padrao que o guard de hoje NAO reconhece como
  "auto-adquirente" (`_funcoes_que_pegam_conexao_propria` casa literalmente `get_pool` +
  `acquire` no corpo da funcao). Ou seja: se uma rota chamasse `pending_invites_count()` de
  dentro de um `async with pool.acquire()` hoje — o mesmo bug que originou o F92 — o guard
  atual NAO acusaria. PR 5 e onde `routes.py` e dividido em 10 modulos e as ~35 queries do
  corpo das rotas (as 11 chamadas a `pending_invites_count` inclusive) passam a usar
  `run_with_reconnect` de proposito — e exatamente ali que faz sentido ensinar o guard a
  reconhecer o novo idioma, porque e ali que o codigo ao redor dele muda de qualquer jeito.
- **F83 (linha 4) vai com C4/reconciliacao idempotente (PR 2) porque o `finally` que
  expoe o limite ainda nao existe.** O guard e por STATEMENT de topo do `finally`
  (`h.chama(stmt, "acquire") e nao h.chama(stmt, "best_effort")`), e um `if` de topo cujo corpo
  tenha um acquire protegido E outro desprotegido passaria verde — o `best_effort` de QUALQUER
  lugar dentro do `if` conta pro `stmt` inteiro (mesma familia "unidade errada" do F57/F58). O
  padrao real do projeto ja existe hoje (`mutations.py:315`, `if reserved: async with
  (best_effort(...), pool.acquire()...)`), mas so tem UM acquire por `if`, entao nao ha
  violacao viva pra apertar agora. PR 2 e onde `record_job_run` de `account_resync.py:252`
  ganha `best_effort` pela PRIMEIRA vez dentro de um `finally` — e o lugar natural pra apertar a
  precisao do guard e provar a ausencia de violacao, junto do proprio codigo novo que exercita a
  forma, nao isolado numa frente que nao mexe em bookkeeping nenhum.
- **DSN (linha 5) vai pra cauda (PR 6) porque o que a tabela nomeia ("`glob` nao-recursivo") ja
  foi corrigido nesta propria rodada** (Task 3, via `h.testes_py(integracao)` — recursivo,
  confirmado por sabotagem em subpacote novo). O que resta e o casamento: `"get_connection_url("
  in linha`, linha a linha, a mesma fragilidade de prosa-conta-como-codigo do F58-C (um
  comentario citando a chamada bateria a mesma substring). Nenhuma das cinco frentes tematicas
  (PR 1-5) mexe em `tests/integration/`, entao nao ha PR natural pra essa precisao — e disso que
  a cauda existe.
- **F113 (linha 8) vai pra cauda (PR 6) pelo mesmo motivo, com a mesma pegadinha.** As duas
  coisas que a tabela nomeia — escopo recursivo e a violacao viva historica — ja foram
  entregues nesta rodada (Task 4: escopo via `h.markdown()`, e a citacao antiga em
  `session-2026-08-19-infra-ci-handoff.md:28` isenta com o marcador `guard-f113:` na linha
  seguinte, verificado no arquivo). O que resta e a mesma fragilidade de casamento por linha
  (as duas substrings do comando quebrado tem que estar na MESMA linha; uma quebra de linha
  escapa) e a fragilidade POSICIONAL do proprio marcador de isencao (a docstring do guard ja
  admite: mover a citacao um numero de linha adiante reabre a violacao em silencio — risco
  aceito, nao corrigido). Nenhuma frente tematica mexe em documentacao; cauda e onde sobra.
- **A linha 9 (`test_ci_realmente_tem_checks`, sem finding numerado) vai pra cauda (PR 6)
  porque hoje ela so prova que o NOME da ferramenta aparece como substring dentro de um bloco
  de step nao-`continue-on-error`** (`_steps_de_check_do_ci()`), nunca que o STEP de fato
  executa o check — um `run:` que so imprimisse um comentario mencionando "ruff check" bateria
  a mesma condicao. Reescrever pra afirmar existencia real de step (nao mencao de nome) nao
  bloqueia nenhuma das cinco frentes tematicas, entao nao ha PR temático que a reivindique.
...
> mais o fix loop da Task 9 que fecha esta entrada e converte o 18o guard (ver acima);
...
> so um lugar novo pro mesmo erro morar. Os 18 guards (17 da varredura original de 05/09 + 1
> achado so na revisao final desta Task 9, antes do merge) foram convertidos preservando
> semantica (zero guard mais estrito, pra nenhuma outra frente travar) — **exceto os dois do
> proprio PR 0** (F58, F91), apertados so depois de provar ausencia de violacao viva (Task 8,
> Step 1: zero linhas nos dois). 7 sabotagens vermelhas e 6 controles positivos verdes
> confirmaram o aperto sem regressao de cobertura — a unica regressao encontrada no processo
> (`.cursor(` como `context_expr` do proprio `with`, que a conversao pra AST deixou de ver por
> um instante) foi medida e fechada no mesmo fix loop.
>
...
> que `EscopoVazioError` dispara quando o scanner nao acha nada. O 18o guard tem prova de
> mordida propria, mais direta: `test_create_pending_audita_dry_run.py::
> test_guard_do_target_count_deriva_escopo_do_harness_nao_de_path_relativo` le a ARVORE da
...
`build_meta_api` **e** `build_facebook_ads_api` foram **removidos** de `client.py`, cada um com os testes que só existiam pra ele — depois da troca de transporte os dois ficavam com zero consumidores, e código morto com teste verde é pior que código morto (parece cobertura). `client.py` **permanece** como módulo de constantes e erros: exporta `MetaAccessDeniedError` (usado por `src/mcp/server.py`) e `META_GRAPH_API_VERSION` (usado por `partnership.py` e pelo transporte novo), e não importa mais nada do SDK.

> ⚠️ **A primeira versão desta entrada afirmava que `build_facebook_ads_api` "segue com consumidor real fora deste caminho".** Era **falso, e ninguém mediu**: `grep -rn "build_facebook_ads_api" src/ tests/` achava a definição e mais nada em `src/`. A função estava na **mesma condição** que justificou remover `build_meta_api` três parágrafos antes — o mesmo texto aplicou o critério a uma e inventou uma exceção pra outra. Instância do padrão do **F187**: a afirmação confortável fica, porque conferir custa um `grep` que ninguém roda. A remoção levou junto os 8 testes da fábrica **e os 2 de sanidade sobre a assinatura do SDK** (`FacebookSession(...)`, `FacebookAdsApi(session=)`) — esses afirmavam a forma de uma API de terceiro que nenhum código nosso chama, teste morto sobre código morto. `CLAUDE.md` e `docs/convencoes/nucleo.md`, que citavam a fábrica pelo F48, foram corrigidos no mesmo commit.
...
### A onda final da revisão de branch — o que só aparecia olhando o conjunto

Quatro dos seis achados eram **defeitos criados ou expostos pelo próprio fix**, invisíveis task a task:

1. **O redator estava cego à forma que a branch acabou de criar.** `_PARAMS_SENSIVEIS` cobria `nome=valor` — a forma de **query string**, o mundo *antes* da troca de transporte. Com a Task 3 o token passou a viajar em `Authorization: Bearer`, e as três formas reais (`Authorization: Bearer X`, minúsculo, e `{'authorization': 'Bearer X'}`) passavam **intactas**, medido. A camada 2 existe pra cobrir o caminho que ninguém previu, e estava cega ao caminho que a camada 1 tinha acabado de abrir. Fechado com um segundo regex **ancorado no ESQUEMA**, não no nome do cabeçalho: o reflexo óbvio — pôr `authorization` na alternação de nomes — foi **medido e produz `Authorization: <REDIGIDO> <token>`**, porque a classe de valor daquele regex para no primeiro espaço; ela redige o literal "Bearer" e deixa o token inteiro passar. Pior que não cobrir, porque o `<REDIGIDO>` no meio da linha *parece* cobertura. Há um teste que reconstrói o regex ingênuo e afirma que ele vaza, senão `"<REDIGIDO>" in resultado` passaria nos dois casos.
2. **O guard de vazamento não pegava a forma idiomática do httpx — e a correção certa era não escrever guard nenhum.** `test_nenhum_caminho_meta_poe_credencial_em_query` casa termos em **literal de string**: `f"...?access_token={token}"` é pego, `http.get(url, params={"access_token": token})` **não** — e `params=` é exatamente o mecanismo do SDK que causou o F190, ou seja, aquele guard sozinho não pegaria o bug original escrito em código nosso. Um 2º eixo chegou a ser implementado e foi **desfeito na mesma sessão**, porque medir antes de confiar valeu uma segunda vez: `tests/unit/test_no_secrets_in_query_params.py` **já cobria isso, e melhor** — verificado com um arquivo-probe, ele varre `src/` inteiro (não só as duas raízes Meta), pega o dict inline **e** o montado numa variável (`p = {...}; params=p`, a forma que o 2º eixo teria declarado como limite), e tem allowlist que só encolhe. Duplicar uma invariante é criar duas fontes de verdade para ela; o valor foi movido para o endereço autoritativo — `appsecret_proof` e `authorization` entraram em `_CHAVES_SECRETAS` (os dois nomes desta família que faltavam lá), a comparação virou case-insensitive (`{"Authorization": …}` é a grafia plausível e escapava), a forma `dict(k=v)` entrou junto com o literal, e o detector ganhou **11 probes de contrato** que ele nunca teve — 7 positivos e **4 negativos**, sendo os negativos os sites reais que uma versão "reforçada" para qualquer `ast.Dict` passaria a acusar: os 4 cabeçalhos `{"Authorization": ...}` (inclusive o próprio fix do F190) e os 3 `{"client_secret": ...}` em `data=`, que são o formato que o F82 **exigiu**. Guard que acusa o próprio fix ensina a contornar o guard. O limite ficou **declarado** no guard de literal, apontando para quem o cobre. Os probes chamam `_achados_na_arvore`, a mesma travessia que roda sobre `src/` — probe que reimplementa a lógica muda junto com ela e para de distinguir código bom de quebrado.
3. **A troca de transporte matou o mapeamento de erro do Meta, e ninguém notou.** `to_friendly_meta_error` mapeava `FacebookRequestError` para as mensagens PT-BR curadas (throttle 4/17/613/80004 + subcode 2635, conexão expirada 458/467/460/463, permissão 190, campo inválido 100). Com o SDK fora do caminho de request, **aquele ramo ficou sem produtor** — e ~10 testes seguiram verdes por cima dele, porque construíam o `FacebookRequestError` à mão. Um throttle Meta chegava ao gestor como **JSON cru em inglês truncado em 200 chars, `retryable=False`**: exatamente o estado que o comentário no topo de `errors.py` dizia ter sido corrigido na PR 6. A Graph devolve `{"error": {"code": N, "error_subcode": M, "message": ...}}` no corpo e o transporte novo **tem** o corpo: `MetaGraphHTTPError` passou a carregar `code`/`subcode`/`mensagem`, e a tabela curada virou `_do_envelope_de_erro`, aplicada a ele. O ramo do SDK saiu, o import saiu, e um assert estrutural impede que voltem sem produtor. `retryable` é **união**, não substituição: a tabela pode *ganhar* (throttle vem em 4xx, e o status sozinho diria "permanente") mas nunca *perde* o que o status já afirmava (5xx é transitório qualquer que seja o code). Dois detalhes que só apareceram implementando: a truncagem **mudou de lugar** (`resp.text[:200]` no chamador cortava o JSON no meio e o envelope se perdia — corpo longo é o caso real, e nenhum teste de corpo curto veria isso), e os subcodes **460 e 463 nunca tiveram teste próprio**, o que a parametrização sobre o frozenset corrigiu de quebra.
4. **`urllib.request.urlretrieve`** era obrigação em aberto da rodada anterior — "consertar OU declarar no docstring", e não foi feito nem um nem outro. É resolvível estaticamente, então não cabia na carve-out de despacho dinâmico: entrou no conjunto com probe próprio. Cobrir metade de um módulo stdlib reproduzia, dentro do `urllib.request`, a mesma assimetria que a rodada anterior fechou entre `httpx` e `aiohttp`.

**O denominador comum dos quatro:** cada um é um sinal que **afirmava mais do que media** depois que o terreno embaixo dele mudou — um redator que dizia cobrir credencial, um guard que dizia cobrir vazamento, uma tabela de mensagens que dizia estar viva, um conjunto que dizia cobrir clientes HTTP. Nenhum deles ficou vermelho; todos ficaram **verdes e vazios**. É a mesma família do F182/F184/F187, agora do lado de quem escreve o fix — e a prova de que **revisão por task não substitui revisão da branch**: os quatro só existem na interação entre tasks que, isoladas, estavam certas.
...
ledger de execução (8 tasks, 10 rulings do coordenador):
...
Houve **duas perdas parciais nesta separação, e as duas em regra SEM âncora própria** — não
em regra com piso errado. A primeira (`aplicar gzip`, acima) foi pega porque a troca de
âncora genérica→específica tropeçou nela por acidente, não por desenho — a âncora nova
existia por outro motivo. A segunda — a cláusula *"Don't adicionar dependência sem checar
'no build step'"*, que dividia bullet com a mecânica do `--universal` — **não tinha âncora
entre as 62** (só a mecânica, âncora `--universal`, tinha sido extraída como regra própria),
e por isso **não existia contraste algum pra ela falhar**: o scan não a via, verde ou
vermelho. Só apareceu porque a revisão da Task 7 leu os 37 bullets removidos um a um contra
os 5 destinos, e um leitor humano notou que metade de um bullet de duas cláusulas tinha
chegado sozinha. **O scan cobre 62 âncoras; o que não tem âncora é invisível a ele, por
construção — não é bug do scan, é o contorno do que ele pode ver.** Fechada nesta sessão
(`docs/convencoes/processo.md`, ao lado da regra do `--universal`, commit `b09351c`), mas o
contorno em si — âncora é a unidade de proteção, não "regra" — fica como propriedade
permanente do mecanismo, não como pendência.
...
bullets/63 cláusulas (12.751 B) → 8 bullets ficam (2.765 B), 37 saem (9.985 B) para os cinco
arquivos de área (`painel.md` 12, `nucleo.md` 11, `dados.md` 4, `testes.md` 5, `processo.md`
5). **78% dos tripwires do `Don't do` saíram do arquivo sempre-carregado**; a mitigação é a
tabela de roteamento (agora declarando que as regras da área foram junto), o bloco de
ponteiros que ficou no `CLAUDE.md`, e o scan.

**Guard:** `tests/unit/test_nenhuma_regra_se_perdeu.py` (2 testes — escopo + as 62 âncoras
